...
XMFLOAT3 Object::old_position() const
{
...
void Object::set_old_position(const XMFLOAT3& value)
{
	old_position_ = value;
}
...
void Object::set_monster_type(MonsterType type)
{
	type_ = type;
}
...
void Object::PrintFrameNamesRecursive(int depth)
{
	std::string indent(depth * 2, ' ');
	std::string printable_name = name_.empty() ? "[NoName]" : name_;
	std::cout << indent << printable_name << std::endl;

	if (child_)
		child_->PrintFrameNamesRecursive(depth + 1);
	if (sibling_)
		sibling_->PrintFrameNamesRecursive(depth);
}
