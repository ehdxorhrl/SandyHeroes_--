...
public:	
	...
	void set_get_text_func(const std::function<std::wstring(std::shared_ptr<Object>)>& func);
...
private:
	...
	std::function<std::wstring(std::shared_ptr<Object>)> get_text_func_{ nullptr }; //텍스트를 가져오는 함수
